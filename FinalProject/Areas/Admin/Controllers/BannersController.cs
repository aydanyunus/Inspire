...
namespace FinalProject.Areas.Admin.Controllers
{
    ...
    public class BannersController : Controller
    {
        ...
        public ActionResult Edit(int? id,[Bind(Include = "Id,Img")] Banner banner, HttpPostedFileBase Photo)
        {
            if (ModelState.IsValid)
            {
                Banner activebanner = db.Banners.Find(id);
                if (activebanner != null)
                {
                    ...
                    if (Photo != null)
                    {
                        if (Photo.ContentLength > 0 && Photo.ContentLength <= 3 * 1024 * 1024)
                        {
                            ...
                            {
                                var path = Path.Combine(Server.MapPath("~/Uploads/"), activebanner.Img);
