...
namespace FinalProject.Areas.Admin.Controllers
{
    public class OurPartnersController : Controller
    {
        ...
        public ActionResult Details(int? id)
        {
            ...
            OurPartner ourPartner = db.OurPartners.Find(id);
            if (ourPartner == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Edit(int? id, [Bind(Include = "Id,Background_img,Content")] OurPartner ourPartner, HttpPostedFileBase Photo)
        {
            if (ModelState.IsValid)
            {
                OurPartner active = db.OurPartners.Find(id);
                if (active != null)
                {
                    ...
                    if (Photo != null)
                    {
                        if (Photo.ContentLength > 0 && Photo.ContentLength <= 3 * 1024 * 1024)
                        {
                            ...
                            )
                            {
                                ...
                                active.Content = ourPartner.Content;
                                db.SaveChanges();
