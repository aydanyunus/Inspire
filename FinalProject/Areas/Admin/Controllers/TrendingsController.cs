...
namespace FinalProject.Areas.Admin.Controllers
{
    ...
    public class TrendingsController : Controller
    {
        ...
        public ActionResult Edit(int? id, [Bind(Include = "Id,Title,Img,View_Count,Written_date")] Trending trending, HttpPostedFileBase Photo)
        {
            if (ModelState.IsValid)
            {
                Trending active = db.Trendings.Find(id);
                if (active != null)
                {
                    ...
                    if (Photo != null)
                    {
                        if (Photo.ContentLength > 0 && Photo.ContentLength <= 3 * 1024 * 1024)
                        {
                            ...
                            )
                            {
                                ...
                                active.Img = fileName;
                                active.Title = trending.Title;
                                active.View_Count = trending.View_Count;
                                active.Written_date = trending.Written_date;
                                db.SaveChanges();
